...
- [ ] 新任务在 `Task.md` 中记录一个经过验证的规范 BCP 47 `Language` 值。
- [ ] 用户偏好为 `zh-CN` 且没有单次覆盖时，新任务记录 `Language: zh-CN`，其叙述性正文使用简体中文。
- [ ] 单次语言覆盖优先于用户偏好，且不改变持久化偏好。
- [ ] 没有单次覆盖和用户偏好时，行为按文档约定稳定回退，并保持向后兼容。
- [ ] 恢复或移交已登记任务时，后续任务工件遵循其 `Language`，不受当前操作者偏好影响。
- [ ] 缺少 `Language` 的 legacy task 确定性地按 `en` 处理，不执行语言猜测或历史工件翻译。
- [ ] 无论正文语言为何，现有英文机器协议标记和生命周期校验继续有效。
- [ ] 非法或不可读取的语言偏好产生明确诊断，不会静默写入仓库级配置。
- [ ] 自动化测试覆盖用户级配置、优先级、`zh-CN` 生成约束、任务语言锁定和 legacy 回退。
