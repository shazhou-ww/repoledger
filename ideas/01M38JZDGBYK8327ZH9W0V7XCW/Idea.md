...
- 为 `whatsnext` invocation 建立明确的 request context，至少区分无选择导航、指定 idea 和
  显式创建新 idea 的意图，同时保持每份 report 只有一个可执行 action。
...
- 改变五态推导、ideaRevision、sibling status 或显式 approval/acceptance 事实模型。
- 在本 idea 内实现完整 Agent session eval harness；可复用其未来产物，但生产 contract 与
  deterministic tests 必须独立成立。

## Implementation acceptance criteria

- CLI 提供文档化、无歧义的方式表达“创建一个新 idea”，即使 primary 中已有 unrelated active
  idea，也能在通过全局 hygiene 后得到对应的单一 create guidance；默认无 intent 行为保持向后
  兼容。
...
- 回归测试覆盖 dirty/conflicted/branch mismatch/ahead/behind/diverged 下的 request context，显式
  ...
  coordinates。
- 所有新增 JSON 字段和 CLI 参数有兼容性决定、human rendering、README、skill 和 package smoke
  coverage；旧 consumer 可忽略新增字段，移除或重命名现有字段必须作为明确 breaking change。
...
- 从已发布 primary 安装 package 与 skill，在 disposable repository/remote 中完成至少一条代表性
  loop：显式 create intent 先被 dirty hygiene 阻挡，清理后保留 request context 并得到 create
  guidance，随后 publication guidance 提供完整安全坐标。
...
- 所有 Git 写入仍由 Agent 使用普通工具显式执行，保留 unknown work 和并发历史，禁止 force-push、
  hard reset、broad clean 或静默 stash。
